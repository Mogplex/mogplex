-- Reserve the marketing page slugs (faq, how-it-works, workflows) at the DB
-- slug-validation layer, mirroring lib/reserved-slugs.ts. Without this, a
-- profile or team could claim one of these slugs and its scoped dashboard
-- would be shadowed by the new static marketing routes.
--
-- Base definition: 20260519161000_reserved_llms_txt_slug.sql. The
-- dashboard-scoped first segments remain unmirrored (pre-existing follow-up
-- noted in lib/reserved-slugs.ts).
...
as $$
  select lower(p_slug) = any (array[
    ...
    'api','auth','cli-auth','conduct','faq','how-it-works','install','login',
    'privacy','request-access','slack','terms','unsubscribe','workflows',
    -- future reservations
...
-- Resolve pre-existing collisions: any profile or team that already owns one
-- of the newly reserved slugs gets the standard numeric-suffix rename (same
...
begin
  for r in
    ...
    where slug in ('faq', 'how-it-works', 'workflows')
    union all
