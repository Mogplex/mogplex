-- Make chat-run liveness derived from data freshness instead of relying on
-- the in-process onFinish/onAbort callbacks to flip status. A stream that is
-- killed mid-flight (deploy, timeout, network drop) leaves an ai_calls row
-- stuck in 'streaming' or 'pending' forever, and the previous version of
-- claim_chat_limit_admission counted those zombies against the per-user
-- concurrent_chat_runs cap, locking the user out.
--
-- Fix:
--   1. Drop and recreate claim_chat_limit_admission with an extra
--      p_stale_threshold_seconds parameter (default 300s, matching
--      ACTIVE_CHAT_STALE_THRESHOLD_MS in lib/interactive-runs.ts). Active
--      chat runs older than that threshold are ignored — the rate-limit
--      check converges on its own.
--   2. Backfill: mark already-stuck rows as failed so the data stops lying
--      about run status, even though step 1 already prevents them from
--      blocking new chats.
--
-- Backwards compatibility: existing callers pass arguments by name and the
-- new parameter has a default, so the OLD app build will keep working
-- against the NEW function while the schema-first deploy completes.

DROP FUNCTION IF EXISTS public.claim_chat_limit_admission(
  UUID, UUID, UUID, TIMESTAMPTZ, UUID, INT, INT, INT, INT, INT, INT, INT
);
...
BEGIN
  ...
  -- Only count chat runs that are still plausibly live. Rows older than the
  -- stale threshold are zombies (the stream was killed before onFinish ran)
  -- and must not block new chats.
  SELECT COUNT(*)
  INTO v_active_chats
...
-- Hygiene backfill: any chat ai_calls row still in pending/streaming past
-- the stale threshold is a zombie from an interrupted stream. Mark it failed
-- so the run history is honest. The new claim function would already ignore
-- these, but leaving them mislabelled corrupts observability surfaces.
UPDATE public.ai_calls
SET status = 'failed',
    error = COALESCE(
      error,
      'Stream interrupted before finalize (older than chat liveness threshold)'
    ),
