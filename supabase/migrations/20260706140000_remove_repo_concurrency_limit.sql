-- Remove the per-repo running-job concurrency limit. Runs are now only
-- gated by the per-installation limit.
--
-- The signature is unchanged: p_repo_id and p_max_running_per_repo are
-- retained (and ignored) so the currently deployed app, which still passes
-- them, keeps working until the app deploy completes.
