-- oauth_client_id can come from the top-level event field, so the rewrite
-- block is reachable even when event -> 'claims' is absent. jsonb_set on a
-- NULL claims object propagates NULL and the hook would return NULL, which
-- fails token issuance instead of no-op'ing. Guard on claims being present.
