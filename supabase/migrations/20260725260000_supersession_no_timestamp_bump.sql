...
-- Stop the reconciler masquerading as a user edit.
--
-- The flows rewrite did `SET draft_graph = ..., updated_at = now()`. Bumping the
-- timestamp made an unattended cron run look like a human edit in every
-- "last edited" surface, and it was inconsistent with the sibling statements in
-- the same function: the agents and profiles updates never touched a timestamp,
-- so half the rewrite was stamped and half was not. The draft_graph change is
-- itself the record that something changed; the timestamp added nothing except
-- a misleading signal. Dropped, which also aligns all three statements.
--
-- Draft/published divergence is deliberate and left as-is: rewriting the draft
-- is what makes the user's *next* publish carry the upgraded model. A flow whose
-- draft previously matched its published version may now show as having
-- unpublished changes, which is the honest state — the draft genuinely differs,
-- and the published version keeps running correctly because the runtime
-- resolver upgrades it at invocation time.
