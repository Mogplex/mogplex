...
Memories are curated context, not a log. Who writes and who reads:

- **Control** (`app/api/control/chat`) reads at the start of every turn via `lib/agents/control-memory-context.ts`: all `semantic` and `procedural` rows that are global or scoped to the selected repo, the newest repo `episodic` rows, and a relevance search against the request. Session rows and rows whose `metadata.source` is `automation` or `harness` never reach the prompt. Control reads the operator's personal and team-tagged rows alike: every row is filtered by `user_id`, and Control's own writers stamp no team id, so the widget's personal/team toggle is a browsing filter rather than a boundary. The block is fail-open and time-boxed (`CONTROL_MEMORY_TIMEOUT_MS`). Control writes through the `memory_write` tool under the `<memory>` policy in `lib/agents/orchestrator/system-prompt.ts`, and promotion (`lib/agents/memory-promotion.ts`) distills durable facts into `semantic`/`procedural` at task completion, from the compaction checkpoint when one exists and otherwise from the finished turn's own record (request, response, tools called) when it is substantive. Every promoted row carries `sourceKind` (`checkpoint` or `turn`) and verbatim evidence.
- **Harness runs** (`app/api/sandbox/[id]/harness`) read the same store for their prompt and write one short `episodic` outcome per run. They must not persist the task prompt: it is run input, already stored on the `ai_call`, and dumping it into `session` used to fill the store with multi-kilobyte rows that were re-injected into later runs.
- **Automations** do not write memories. Run outcomes belong to the job-run tables; the 11k-row backlog they produced before 2026-09-18 is removable with the widget's Prune action (`/api/memories/actions` `prune_noise`).
- The memories widget (`components/library/context-section.tsx`, also the `/agents/context` page) reads `/api/memories`, which returns 50 rows per lane plus exact `counts` from `lib/memories-maintenance.ts`.

Do not add a new memory writer that stores transcripts, prompts, or per-run output. If a surface needs run history, query the run tables.
...
`lib/skill-catalog/` decides which skills a run may draw on: the acting user's library, minus the skills the repo excludes, plus the repo's own. Read [docs/skills.md](./docs/skills.md) before touching it. A user invokes a skill by slug on any surface (`/slug` as the first word, `$slug` anywhere; `resolveInvokedSkills` is the only parser), and an agent discovers one through the prompt index plus `find_skills` / `load_skill` (`lib/agents/tools/skills.ts`, capability `tools.skills`). The binding rules: a catalog that fails to load never blocks a run; the user's message is never rewritten, skill text travels beside it; slugs are derived from names, never stored. Every conversational surface gets this from `createChatModelStream`, and Control from `loadControlKnowledgeContext` beside its memory block; a new surface that builds its own prompt must call the catalog itself.

### Decision layer

`lib/decisions/` asks an evaluation model closed questions about program state (command risk, hidden tool failures, unsupported claims, loops, memory promotion, and, observing only, which skills and injected memories a request needed) and records every answer in `decision_events`. Read [docs/decisions.md](./docs/decisions.md) before touching it. The binding rules: `decide()` fails open and never throws; a decision may only add caution (an approval, a note) and never relaxes `policy.ts` or the shell guard; nothing in it may end or shorten a run; question wording and thresholds live only in `definitions.ts` and `definitions-selection.ts` with a `version` bump on every change; every call site puts `teamId` in its scope so the account's Run checks switch applies; customer-facing text never names the evaluation provider. The one authored decision is the automation Classify node (`classify()` in `lib/decisions/classify.ts`): its question lives in the customer's flow graph, and it does not fail open, because the author chose that node to pick a branch. An outage fails the node so the flow's `error` handle can route it. Tests inject evaluators through `deps`; the real evaluator is inert under Vitest.
...
- Follow the [migration rules](#migrations-never-apply-schema-changes-out-of-band): preserve compatibility until all retained app releases and dependent Trigger workers retire
