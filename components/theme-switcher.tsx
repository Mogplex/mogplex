...
const ACTIVE_WIDTH: Record<ThemePreference, number> = {
  dark: 62,
  light: 62,
  system: 82,
};
...
function indicatorXFor(theme: ThemePreference): number {
  if (theme === "dark") return 0;
  if (theme === "light") return INACTIVE_WIDTH;
  return INACTIVE_WIDTH * 2;
}

function widthFor(theme: ThemePreference, active: string | undefined): number {
  return active === theme ? ACTIVE_WIDTH[theme] : INACTIVE_WIDTH;
}
...
const ICONS: Record<ThemePreference, React.ComponentType> = {
  dark: DarkIcon,
  light: LightIcon,
  system: SystemIcon,
};
...
export function ThemeSwitcher() {
  const { theme, selectTheme } = useThemeSelector();
  ...
  if (!mounted) {
    return (
      <div className="border-border bg-secondary relative flex h-8 min-w-max rounded-lg border p-1 opacity-50" />
    );
  }
  ...
  const activeTheme: ThemePreference =
    theme === "dark" || theme === "light" || theme === "system"
      ? theme
      : "system";
  const indicatorX = indicatorXFor(activeTheme);
  const indicatorWidth = ACTIVE_WIDTH[activeTheme];
  ...
  return (
    <div
      ...
      className="border-border bg-secondary relative flex min-w-max rounded-lg border p-1"
    >
      <div
        aria-hidden="true"
      ...
      {THEME_OPTIONS.map((option) => {
        ...
        const width = widthFor(option, activeTheme);
        return (
          <button
            key={option}
            type="button"
          ...
          >
            ...
            {isActive && (
              <span className="font-mono text-[12px] leading-[100%] tracking-[-0.015rem] whitespace-nowrap uppercase">
                {option}
              </span>
            )}
          </button>
