...
export function ModelFallbackSettings({ defaultModel }: { defaultModel: string }) {
  ...
  return <section aria-labelledby="fallback-models-heading" className="min-w-0 space-y-3">
    <div>
      <div className="flex items-center justify-between gap-3">
        <h2 id="fallback-models-heading" className="text-sm font-medium">Fallback models</h2>
        <span className="text-xs tabular-nums text-muted-foreground">{selected.length} / {ACCOUNT_FALLBACK_MODEL_MAX_COUNT}</span>
      </div>
      <p className="text-muted-foreground mt-1 text-xs leading-5">Account-wide for AI Gateway. Automation overrides take priority. Standard model prices apply.</p>
    </div>
    {isLoading ? <p role="status" className="text-xs text-muted-foreground">Loading fallback models…</p> : error ?
      ...
      <fieldset disabled={saving} className="min-w-0 space-y-2">
        ...
        {selected.map((id, index) => <div key={id} className="flex items-center gap-2">
          <span aria-hidden className="w-4 shrink-0 text-center text-xs tabular-nums text-muted-foreground">{index + 1}</span>
          ...
          <Button type="button" size="icon" className="size-8 shrink-0" variant="ghost" disabled={index === 0} title="Move up" aria-label={`Move fallback ${index + 1} up`}
            onClick={() => { const next = [...selected]; [next[index - 1], next[index]] = [next[index]!, next[index - 1]!]; change(next); }}><ArrowUp className="size-4" /></Button>
          <Button type="button" size="icon" className="size-8 shrink-0" variant="ghost" title="Remove fallback" aria-label={`Remove fallback ${index + 1}`}
            onClick={() => change(selected.filter((_, at) => at !== index))}><X className="size-4" /></Button>
        </div>)}
        ...
        <div className="flex min-h-9 flex-wrap items-center gap-2 pt-1">
          <Button size="sm" variant={draft === null ? "outline" : "default"} disabled={draft === null} onClick={() => void save()}>{saving ? "Saving…" : "Save fallbacks"}</Button>
          ...
          {message && <p role="status" className="flex items-center gap-1 text-xs text-muted-foreground"><Check aria-hidden className="size-3.5" />{message}</p>}
        </div>
      </fieldset>}
