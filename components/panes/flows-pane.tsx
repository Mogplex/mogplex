...
import {
  ...
  ConditionInspector, ClassifyInspector, ParallelInspector, JoinInspector, DelayInspector, EndInspector,
          ...
          {assistantPanelOpen && selectedFlow && draftGraph ? <FlowAssistantPanel key={selectedFlow.id} flowId={selectedFlow.id} graph={draftGraph} onApplyGraph={applyAssistantGraph} /> : !selectedFlow || !selectedNode ? (
            ...
            <div className="flows-panel-shell">
              ...
              <div key={selectedFlow.id} ref={inspectorScrollRef} data-testid="flows-inspector-scroll" className="@container min-h-0 flex-1 space-y-5 overflow-y-auto px-4 py-5">
                ...
                <div className="space-y-5 border-t border-border pt-5"><div className="ui-kicker">Flow</div><section className="space-y-2"><label htmlFor="flow-description" className="ui-section-title">Description</label><Textarea id="flow-description" value={draft?.description ?? ""} onChange={(e) => updateDraft((c) => ({ ...c, description: e.target.value }), { mergeKey: "flow-description" })} rows={3} placeholder="Describe what this flow should accomplish." className="bg-input/40" /></section><section className="space-y-2"><div className="ui-section-title">Notes</div><Textarea value={draft?.notes ?? ""} onChange={(e) => updateDraft((c) => ({ ...c, notes: e.target.value }), { mergeKey: "flow-notes" })} rows={8} placeholder="Capture intent, guardrails, and context for this flow." /></section><section><Button type="button" variant="outline" size="sm" onClick={() => toggleAssistantPanel()} className="w-full">Open assistant</Button></section></div>
              </div>
            </div>
          )}
        </aside>
