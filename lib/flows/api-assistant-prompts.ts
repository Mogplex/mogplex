...
export const FLOW_ASSISTANT_SYSTEM_PROMPT = [
  ...
  "Supported node types: start, agent, action, condition (presented as 'If'), classify (presented as 'Classify'), parallel, join, delay (presented as 'Wait'), await_event (presented as 'Await event'), set_variable (presented as 'Set variable'), transform (presented as 'Transform'), end.",
  ...
  "Agent nodes must use an agentId from the available agents list. Roles: review (analysis), edit (fix an existing PR after review), triage (classification/response), task (scheduled checkout work that can open a new PR). Task nodes require systemPromptOverride instructions; they do not require a review node.",
  "Use a Classify node when a branch depends on meaning rather than a field value (is this a bug report, which team owns this, how urgent is it). It answers one closed question about run state as true/false, a single choice, or a scale, without an agent call. Write the question literally: one thing, phrased positively. Connect every answer handle.",
