...
import { persistControlStartupFailure } from "./lifecycle";
import { executeControlChatRequest } from "./execute";
import {
 ...
 */
export async function runAuthorizedControlChat(
  req: Request,
  userId: string,
  options: {
  ...
  try {
    ...
    if (options.background) {
      const expected = options.background.expectedContext;
      if (
        body.repoId !== expected.repoId ||
        body.missionId !== expected.missionId ||
        body.model !== expected.model ||
        body.mode !== "run"
      )
        throw new ControlChatSessionContextError(
          "The mission context changed after the worker handoff. Continue from the current conversation.",
          404
        );
      await options.background.assertCurrent();
    }
  } catch (error) {
    const status =
  ...
  try {
    ...
    if (result.completion) options.onCompletion(result.completion);
