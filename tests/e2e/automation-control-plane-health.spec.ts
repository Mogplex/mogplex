...
for (const failedRuns of [0, 12]) {
  test(`automation history does not request user action (${failedRuns} failed runs)`, async ({
    page,
  }, testInfo) => {
    ...
    await page.route("**/api/observability/stats*", (route) =>
      fulfillJson(route, {
        summary: {
          ...
          job_runs_failed_in_range: failedRuns,
          job_runs_repaired_in_range: 19,
          job_runs_concluded_in_range: 804,
          job_runs_success_rate_in_range: failedRuns > 0 ? 75 : 100,
    ...
    const health = page.getByRole("region", { name: "Automation health" });
    await expect(
      health.getByText(failedRuns > 0 ? "Issues recorded" : "Healthy", {
        exact: true,
      })
    ).toBeVisible();
    await expect(
      health.getByText("Needs attention", { exact: true })
    ).toHaveCount(0);
    await expect(
      health.getByText("Action required", { exact: true })
    ).toHaveCount(0);
    await expect(
      health.getByText("Run history", { exact: true })
    ).toBeVisible();
    await expect(
      health.getByText(
        "0 running runs · 0 pending runs · 7626 tracked all time"
      )
    ).toBeVisible();
    await expect(
      page.getByText("Recent pressure", { exact: true })
    ).toHaveCount(0);
    if (failedRuns > 0) {
      await expect(health.getByText("12 runs remain failed")).toBeVisible();
      await health.screenshot({
        path: testInfo.outputPath("automation-health.png"),
      });
    } else {
      await expect(page.getByText("No failed or delayed runs")).toBeVisible();
      await expect(
        page.getByText("Start failures", { exact: true })
      ).toBeVisible();
      await expect(page.getByText("2 failed start attempts")).toBeVisible();
      await expect(
        page.getByText("3 delayed start attempts retried automatically.")
      ).toBeVisible();
    }
  });
}
