...
function readEnvValue(name: string): string | undefined {
  ...
  // Playwright specs don't inherit Next's .env.local loading — pull the value
  // out directly so local runs hit the same database the dev server uses.
  try {
...
const databaseUrl =
  readEnvValue("DATABASE_URL") ?? readEnvValue("mogplex_DATABASE_URL");
