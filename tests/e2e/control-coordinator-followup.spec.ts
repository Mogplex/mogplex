...
for (const ending of ["cancelled", "timed out"] as const) {
  test(`worker handoff updates the conversation without another prompt, and ${ending} survives reload`, async ({
    page,
  }, testInfo) => {
    ...
    try {
      ...
      if (ending === "cancelled") {
        ...
        await superviseControlContinuation(
          { userId: f.owner, continuationId: continuation.id },
          "runtime",
          {
            client,
            waitForWorker: async () => ({
        ...
        await expect(status).toContainText("reached its time limit");
        await expect(
          status.getByRole("button", { name: "Stop follow-up" })
        ).toHaveCount(0);
      }
      await expect(status).toContainText(finalLabel);
      expect(
