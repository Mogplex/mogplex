...
async function loadFlowAssistantChatRoute() {
...
const allowLimits = (async () => ({
  allowed: true,
  claimId: "claim-1",
})) as never;

const noopRelease = (async () => true) as never;
const noopRecord = (async () => undefined) as never;

function createReleaseProbe() {
