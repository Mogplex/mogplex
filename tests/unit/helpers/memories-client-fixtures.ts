...
export function makeFakeSupabase(rows: Memory[] = []) {
  ...
  // Each `from()` call gets its own chain state so parallel queries (e.g.
  // Promise.all over lanes) cannot interleave filters, matching the real
  // client where every `from()` returns a fresh builder.
  const createBuilder = (currentTable: string) => {
